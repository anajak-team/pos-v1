...
export const PosView: React.FC<PosViewProps> = ({
  ...
  // FIX: Support iOS by removing dependency on experimental BarcodeDetector and using Html5Qrcode via global script
  ...
  const handleSearchKeyDown = async (e: React.KeyboardEvent<HTMLInputElement>) => {
      // Barcode scanners typically send an 'Enter' key after scanning the code
      if (e.key === 'Enter') {
          e.preventDefault();
          ...
          if (productToAdd) {
              ...
              setSearch(''); // Clear for continuous scanning
              showToast(`Added: ${productToAdd.name}`, 'success');
          } else {
              // 3. Fallback: If filter resulted in exactly one item, select it
              if (filteredProducts.length === 1) {
                  ...
                  if (filteredProducts.length === 0) {
                      // Check if it looks like a barcode (numeric, > 5 chars) to suggest adding it
                      if (/^\d{5,}$/.test(term)) {
                      ...
                      // Ambiguous search, do nothing (let user pick from filtered list)
                  }
              }
          }
      }
  ...
  const handleCheckout = async () => {
    ...
    // FIX: Add missing 'type' property to satisfy the Transaction interface.
  ...
  const handleScan = useCallback((code: string) => { 
      // Debounce logic handled by Html5Qrcode somewhat, but good to check
      if(code === unrecognizedBarcode) return; // Prevent loop on modal open

      const product = products.find(p => p.barcode === code); 
      if (product) { 
          addToCart(product); 
          setLastScannedMsg(`Added: ${product.name}`); 
          // Optional: slight pause could be added here if desired
          setTimeout(() => setLastScannedMsg(null), 2000); 
      } else { 
          playSystemSound('error'); 
          setIsScannerOpen(false); // Close camera to show input form
          setUnrecognizedBarcode(code); 
          setIsQuickAddOpen(true); 
      } 
  }, [products, unrecognizedBarcode]);

  // Effect to manage Html5Qrcode lifecycle
  useEffect(() => {
    let html5QrCode: any;
    
    if (isScannerOpen) {
        // Wait a tick for DOM to render the 'reader' div
    ...
    <div className="flex flex-col lg:flex-row h-full gap-4 relative">
           ...
           {filteredProducts.length === 0 ? (
             ...
             <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-4 gap-3 pb-24 lg:pb-4">
                {filteredProducts.map(product => (
                   <ProductCard key={product.id} product={product} onAdd={addToCart} currency={settings.currency} />
                ))}
             </div>
           )}
        </div>
      ...
      {!showCheckout && cart.length > 0 && (
        ...
        >
          ...
          <span className="font-bold text-lg">{settings.currency}{total.toFixed(2)}</span>
        </button>
      )}

      {/* Cart Backdrop - Mobile Only */}
      {showCheckout && (
        <div className="lg:hidden fixed inset-0 bg-black/40 backdrop-blur-sm z-40 animate-fade-in" onClick={() => setShowCheckout(false)} />
      )}

      {/* Cart (Sidebar on Desktop, Modal on Mobile) */}
      <div className={`
          flex flex-col h-full shrink-0 overflow-hidden
          w-full lg:w-[380px] xl:w-[420px] 
          bg-white/60 dark:bg-slate-900/60 backdrop-blur-2xl rounded-3xl shadow-2xl border border-white/20 dark:border-white/10
          
          /* Mobile Modal State */
          fixed inset-0 z-50 rounded-t-3xl lg:rounded-3xl h-[90vh] lg:h-full top-auto lg:top-0
      ...
      `}>
         ...
         <div className="p-5 border-b border-white/10 bg-white/10 dark:bg-black/10 flex justify-between items-center shrink-0">
            ...
            <div className="flex gap-2">
                ...
                <button onClick={() => setShowCheckout(false)} className="lg:hidden p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-white/10 rounded-xl"><X size={20} /></button>
            </div>
         </div>
                ...
                <>
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between items-center text-slate-500 dark:text-slate-400">
                        ...
                        {discountAmount > 0 && (
                          <span className="font-bold text-red-500">- {settings.currency}{discountAmount.toFixed(2)}</span>
                        )}
                      </div>
                      ...
                      <div className="flex justify-between items-end pt-2 border-t border-black/5 dark:border-white/5">
                          ...
                          <div className="text-right">
                              <span className="font-bold text-2xl text-slate-900 dark:text-white block leading-none">{settings.currency}{total.toFixed(2)}</span>
                              {settings.secondaryCurrency && (
                                  <span className="text-xs text-slate-500 font-medium">
                                  ≈ {settings.secondaryCurrency} {(total * settings.exchangeRate).toLocaleString()}
                                  </span>
                              )}
                          </div>
                      </div>
                    ...
                    <div className="animate-fade-in">
                        ...
                        {cashReceived && parseFloat(cashReceived) >= total && (
                            <div className="mt-2 flex justify-between items-center p-2 bg-green-500/10 rounded-lg text-green-700 dark:text-green-400 text-sm font-bold">
                    ...
                    disabled={cart.length === 0 || (paymentMethod === 'cash' && (!cashReceived || parseFloat(cashReceived) < total)) || processing}
