...
export const LandingPageBuilderView: React.FC<LandingPageBuilderProps> = ({ settings, onSave, onBack }) => {
  ...
  return (
    <div className="fixed inset-0 z-[50] overflow-y-auto bg-slate-50 dark:bg-slate-950 animate-fade-in no-scrollbar">
      <div className="min-h-full w-full">
        <div className="max-w-4xl mx-auto space-y-6 pb-20 pt-4 px-4 md:px-6">
          <div className="flex items-center justify-between sticky top-4 z-20 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-4 rounded-2xl border border-white/20 dark:border-white/5 shadow-sm">
            <div className="flex items-center gap-3">
                <button onClick={onBack} className="p-2 hover:bg-black/5 dark:hover:bg-white/10 rounded-xl transition-colors">
                    <ArrowLeft size={24} className="text-slate-700 dark:text-white" />
                </button>
                <div>
                    <h2 className="text-xl md:text-2xl font-bold text-slate-800 dark:text-slate-100">Page Builder</h2>
                    <p className="text-slate-500 dark:text-slate-400 text-xs md:text-sm">Customize storefront</p>
                </div>
            </div>
            <button onClick={handleSave} className="bg-primary text-white px-4 md:px-6 py-2.5 rounded-xl font-bold shadow-lg flex items-center gap-2 hover:bg-blue-600 transition-colors text-sm md:text-base">
