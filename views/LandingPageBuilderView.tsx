...
import { ArrowLeft, Globe, Gift, Eye, Shield, EyeOff, Trash2, Plus, Save, Wrench, CreditCard, Video, Users, User, Layout, GripVertical, Check, PlusCircle, Sparkles, Upload } from 'lucide-react';
...
export const LandingPageBuilderView: React.FC<LandingPageBuilderProps> = ({ settings, onSave, onBack }) => {
  ...
  const handleAddSection = (type: LandingPageSection['type']) => {
      ...
      const defaultContent: Record<string, any> = {
          ...
          video: { title: 'Watch Now', videoUrl: '', overlayTitle: 'Video Title', overlaySubtitle: 'A short description', features: [{ title: 'Key Benefit', desc: 'Short description', icon: 'Check' }] },
  ...
  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>, userIndex: number, sectionId: string) => {
      ...
      reader.onloadend = () => {
          const sections = (localSettings.landingPage?.sections || []).map(s => {
              if (s.id === sectionId) {
                  const newUsers = [...(s.content.users || [])];
                  if (newUsers[userIndex]) {
                      newUsers[userIndex] = { ...newUsers[userIndex], logo: reader.result as string };
                  }
                  return { ...s, content: { ...s.content, users: newUsers } };
              }
              return s;
          });
          updateSections(sections);
          ...
          case 'video':
              return (
                  <div className="space-y-4">
                      ...
                      <div className="flex justify-between items-center mb-2 pt-4 border-t border-slate-200 dark:border-white/10">
                          <h4 className="font-bold text-sm">Video Features</h4>
                          <button onClick={() => handleUpdateContent(section.id, { features: [...(section.content.features || []), { title: 'New Feature', desc: 'Description', icon: 'Zap' }] })} className="text-xs bg-primary/10 text-primary px-3 py-1.5 rounded-lg hover:bg-primary/20 transition-colors flex items-center gap-1 font-bold"><Plus size={14} /> Add Feature</button>
                      </div>
                      <div className="space-y-4 max-h-96 overflow-y-auto">
                          {(section.content.features || []).map((item: any, idx: number) => (
                              <div key={idx} className="p-4 bg-slate-50 dark:bg-black/20 rounded-2xl border border-white/30 dark:border-white/5 relative group shadow-sm">
                                  <button onClick={() => { const newItems = [...(section.content.features || [])]; newItems.splice(idx, 1); handleUpdateContent(section.id, { features: newItems }); }} className="absolute top-3 right-3 p-1.5 text-slate-400 hover:text-red-500 rounded-lg hover:bg-red-500/10 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={16} /></button>
                                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-3 pr-8">
                                      <InputGroup label="Title" value={item.title} onChange={(v) => { const newItems = [...(section.content.features || [])]; newItems[idx].title = v; handleUpdateContent(section.id, { features: newItems }); }} />
                                      <div className="space-y-1.5">
                                          <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase ml-1">Icon</label>
                                          <select className={commonInputClass} value={item.icon} onChange={(e) => { const newItems = [...(section.content.features || [])]; newItems[idx].icon = e.target.value; handleUpdateContent(section.id, { features: newItems }); }}>
                                              {['Rocket', 'QrCode', 'Languages', 'Store', 'Package', 'ShieldCheck', 'Zap', 'Globe', 'Gift', 'Settings', 'ShoppingCart', 'BarChart3', 'Wrench', 'Sliders', 'LayoutGrid', 'Play', 'Check'].map(ic => <option key={ic} value={ic}>{ic}</option>)}
                                          </select>
                                      </div>
                                  </div>
                                  <div className="mt-3">
                                      <InputGroup label="Description" value={item.desc} onChange={(v) => { const newItems = [...(section.content.features || [])]; newItems[idx].desc = v; handleUpdateContent(section.id, { features: newItems }); }} />
                                  </div>
                              </div>
                          ))}
                      </div>
                  ...
                  <div className="space-y-4">
                      ...
                      <div className="space-y-3 max-h-96 overflow-y-auto">
                          {(section.content.users || []).map((user: any, idx: number) => (
                              <div key={idx} className="p-4 bg-slate-50 dark:bg-black/20 rounded-2xl border border-white/20 dark:border-white/5 relative group">
                                   <button onClick={() => { const newUsers = [...section.content.users]; newUsers.splice(idx, 1); handleUpdateContent(section.id, { users: newUsers }); }} className="absolute top-3 right-3 p-1.5 text-slate-400 hover:text-red-500 rounded-lg hover:bg-red-500/10 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={16} /></button>
                                  <div className="space-y-3">
                                      ...
                                      <div className="space-y-1.5">
                                          <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase ml-1">Logo</label>
                                          <div className="flex gap-2">
                                              <input 
                                                  type="text" 
                                                  value={user.logo || ''} 
                                                  onChange={(e) => { const newUsers = [...section.content.users]; newUsers[idx].logo = e.target.value; handleUpdateContent(section.id, { users: newUsers }); }} 
                                                  className={commonInputClass}
                                                  placeholder="Image URL"
                                              />
                                              <label className="p-3 bg-white/50 dark:bg-black/20 border border-white/30 dark:border-white/10 rounded-2xl cursor-pointer hover:bg-white/80 dark:hover:bg-white/10 transition-colors shrink-0 flex items-center justify-center">
                                                  <Upload size={20} className="text-slate-600 dark:text-slate-300" />
                                                  <input type="file" accept="image/*" className="hidden" onChange={(e) => handleLogoUpload(e, idx, section.id)} />
                                              </label>
                                          </div>
                                          {user.logo && (
                                              <div className="mt-2 p-2 bg-white/80 dark:bg-black/40 rounded-xl w-fit border border-slate-200 dark:border-white/10">
                                                  <img src={user.logo} alt="Preview" className="h-8 object-contain" />
                                              </div>
                                          )}
                                      </div>
                                  </div>
  ...
  return (
    <div className="flex flex-col h-screen bg-transparent overflow-hidden">
      <div className="h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-200 dark:border-white/10 flex items-center justify-between px-6 shrink-0 z-20">
      ...
      <div className="flex-1 flex overflow-hidden">
          <div className="w-80 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-r border-slate-200 dark:border-white/10 flex flex-col overflow-hidden shrink-0 z-10">
          ...
          <div className="flex-1 bg-white/30 dark:bg-black/20 backdrop-blur-sm overflow-y-auto p-8 no-scrollbar">
              {editingSectionId ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center text-slate-600 dark:text-slate-300 opacity-80 drop-shadow-md">
