...
import { Move, ZoomOut, ZoomIn, Check, X, Camera, ScanBarcode, Save, Box, Wand2, ImageIcon, Printer, Loader2 } from 'lucide-react';
...
const ImageCropper = ({ imageSrc, onCrop, onCancel }: { imageSrc: string, onCrop: (croppedImage: string) => void, onCancel: () => void }) => {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      if (aspect > 1) { drawHeight = VIEWPORT_SIZE; drawWidth = VIEWPORT_SIZE * aspect; } 
      else { drawWidth = VIEWPORT_SIZE; drawHeight = VIEWPORT_SIZE / aspect; }
  ...
  const draw = (img: HTMLImageElement, currentScale: number, currentPos: { x: number, y: number }) => {
    ...
    const aspect = img.width / img.height;
    let drawWidth, drawHeight;
    if (aspect > 1) { drawHeight = VIEWPORT_SIZE * currentScale; drawWidth = VIEWPORT_SIZE * aspect * currentScale; } 
    else { drawWidth = VIEWPORT_SIZE * currentScale; drawHeight = (VIEWPORT_SIZE / aspect) * currentScale; }
...
const CameraCapture = ({ onCapture, onCancel }: { onCapture: (img: string) => void, onCancel: () => void }) => {
  ...
  useEffect(() => {
    ...
    const initCamera = async () => { try { stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' } }); if (videoRef.current) { videoRef.current.srcObject = stream; } } catch (err) { console.error("Error accessing camera:", err); onCancel(); } };
    initCamera();
    return () => { if (stream) { stream.getTracks().forEach(track => track.stop()); } };
  }, [onCancel]);
  const capture = () => { if (videoRef.current) { const canvas = document.createElement('canvas'); canvas.width = videoRef.current.videoWidth; canvas.height = videoRef.current.videoHeight; const ctx = canvas.getContext('2d'); if (ctx) { ctx.drawImage(videoRef.current, 0, 0); const dataUrl = canvas.toDataURL('image/jpeg', 0.9); onCapture(dataUrl); } } };
  return (
    <div className="fixed inset-0 z-[70] bg-black flex flex-col items-center justify-center">
      <video ref={videoRef} autoPlay playsInline className="w-full h-full object-cover" />
      <div className="absolute bottom-8 w-full flex justify-center items-center gap-10"><button type="button" onClick={onCancel} className="p-4 bg-white/20 backdrop-blur-md rounded-full text-white hover:bg-white/30 transition-colors"><X size={24} /></button><button type="button" onClick={capture} className="p-1 rounded-full border-4 border-white transition-transform active:scale-95"><div className="w-16 h-16 bg-white rounded-full"></div></button></div>
    </div>
  );
};
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
        ...
        try {
            ...
            html5QrCode.start({ facingMode: "environment" }, { fps: 20, aspectRatio: 1.0 }, (decodedText: string) => { if (scannerRef.current && !scanSuccess) { setScanSuccess(true); setTimeout(() => { onScan(decodedText); }, 500); } }, () => {}).catch((err: any) => console.error(err));
        } catch (e) { console.error(e); }
    }, 100);
    return () => { clearTimeout(timer); if (scannerRef.current) try { scannerRef.current.stop(); } catch(e) {} };
  }, []);
  return (
    <div className="fixed inset-0 z-[100] bg-black/95 flex flex-col items-center justify-center animate-fade-in">
        <button onClick={onClose} className="absolute top-6 right-6 p-3 bg-white/10 backdrop-blur-md rounded-full text-white hover:bg-white/20 transition-colors z-50"><X size={24} /></button>
        <div className="relative w-full max-w-sm aspect-square bg-black rounded-3xl overflow-hidden shadow-2xl border border-white/10 mx-6"><div id="product-modal-scanner-reader" className="w-full h-full"></div></div>
    </div>
  );
};
      ...
      <div className="bg-white/80 dark:bg-slate-900/90 backdrop-blur-2xl rounded-[2rem] shadow-2xl w-full max-w-lg overflow-hidden border border-white/20 my-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div className={`flex flex-col items-center justify-center p-1 border-2 border-dashed rounded-2xl transition-colors relative h-36 overflow-hidden ${formData.image ? 'border-transparent bg-slate-100 dark:bg-black/40' : 'border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-black/20'}`}>
              {formData.image ? (
                  <div className="relative w-full h-full group"><img src={formData.image} alt="Preview" className="w-full h-full object-contain rounded-xl" /><button type="button" onClick={handleRemoveImage} className="absolute top-2 right-2 p-1.5 bg-red-500 text-white rounded-full shadow-lg hover:bg-red-600 transition-transform hover:scale-110 z-20" title="Remove Image"><X size={14} /></button></div>
              ) : (
                  <div className="flex gap-8 items-center justify-center w-full h-full">
                      <label className="flex flex-col items-center gap-2 cursor-pointer group p-2 rounded-xl hover:bg-black/5 dark:hover:bg-white/5 transition-colors"><div className="p-3 bg-blue-500/10 text-blue-600 rounded-full group-hover:scale-110 transition-transform"><ImageIcon size={24} /></div><span className="text-xs font-bold text-slate-500 dark:text-slate-400">Upload</span><input type="file" accept="image/*" onChange={handleImageChange} className="hidden" /></label>
                      <div className="w-px h-12 bg-slate-300 dark:bg-slate-700"></div>
                      <button type="button" onClick={() => setIsCameraOpen(true)} className="flex flex-col items-center gap-2 cursor-pointer group p-2 rounded-xl hover:bg-black/5 dark:hover:bg-white/5 transition-colors"><div className="p-3 bg-purple-500/10 text-purple-600 rounded-full group-hover:scale-110 transition-transform"><Camera size={24} /></div><span className="text-xs font-bold text-slate-500 dark:text-slate-400">Camera</span></button>
                  </div>
              )}
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div className="space-y-1"><label className="text-xs font-bold text-slate-500 dark:text-slate-400">Name</label><input required type="text" className="w-full p-3 rounded-xl bg-white/50 dark:bg-black/20 border border-white/30 dark:border-white/10 outline-none" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} /></div><div className="space-y-1"><label className="text-xs font-bold text-slate-500 dark:text-slate-400">Category</label><select className="w-full p-3 rounded-xl bg-white/50 dark:bg-black/20 border border-white/30 dark:border-white/10 outline-none" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value })}>{categories.map(c => <option key={c} value={c}>{c}</option>)}</select></div></div>
          <div className="grid grid-cols-2 gap-4"><div className="space-y-1"><label className="text-xs font-bold text-slate-500 dark:text-slate-400">Price</label><input required type="number" step="0.01" className="w-full p-3 rounded-xl bg-white/50 dark:bg-black/20 border border-white/30 dark:border-white/10 outline-none" value={formData.price || ''} onChange={e => setFormData({...formData, price: parseFloat(e.target.value)})} /></div><div className="space-y-1"><label className="text-xs font-bold text-slate-500 dark:text-slate-400">Cost (COGS)</label><input type="number" step="0.01" className="w-full p-3 rounded-xl bg-white/50 dark:bg-black/20 border border-white/30 dark:border-white/10 outline-none" value={formData.cost || ''} onChange={e => setFormData({...formData, cost: parseFloat(e.target.value)})} placeholder="0.00" /></div></div>
          ...
          <div className="space-y-1"><label className="text-xs font-bold text-slate-500 dark:text-slate-400">Barcode</label><div className="relative"><input type="text" className="w-full pl-3 pr-10 p-3 rounded-xl bg-white/50 dark:bg-black/20 border border-white/30 dark:border-white/10 outline-none" value={formData.barcode} onChange={e => setFormData({...formData, barcode: e.target.value})} placeholder="Scan or type..." /><button type="button" onClick={() => setIsBarcodeScannerOpen(true)} className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 text-slate-400 hover:text-primary hover:bg-blue-500/10 rounded-lg transition-colors"><ScanBarcode size={20} /></button></div></div>
          <div className="space-y-1"><div className="flex justify-between items-center"><label className="text-xs font-bold text-slate-500 dark:text-slate-400">Description</label><button type="button" onClick={handleGenerateDesc} disabled={isGenerating || !formData.name} className="text-[10px] flex items-center gap-1 text-primary font-bold disabled:opacity-50 hover:underline"><Wand2 size={10}/> AI Auto-Write</button></div><textarea className="w-full p-3 rounded-xl bg-white/50 dark:bg-black/20 border border-white/30 dark:border-white/10 outline-none min-h-[80px] text-sm" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} placeholder="Product details..." /></div>
        ...
        {isBarcodeScannerOpen && <BarcodeScanner onScan={(code) => { setFormData(prev => ({ ...prev, barcode: code })); setIsBarcodeScannerOpen(false); showToast(`Scanned: ${code}`, 'success'); }} onClose={() => setIsBarcodeScannerOpen(false)} />}
    ...
    <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl rounded-3xl border border-white/30 dark:border-white/10 overflow-hidden">
            <table className="w-full text-left text-sm">
                <thead className="bg-white/20 dark:bg-white/5 border-b border-white/10"><tr><th className="p-4">ID</th><th className="p-4">Date</th><th className="p-4">Customer</th><th className="p-4">Total</th><th className="p-4">Method</th><th className="p-4">Action</th></tr></thead>
                <tbody className="divide-y divide-white/10">{transactions.map((t) => (<tr key={t.id} className="hover:bg-white/10"><td className="p-4 font-mono text-xs">{t.id.slice(-6)}</td><td className="p-4">{new Date(t.date).toLocaleString()}</td><td className="p-4">{t.customerName || 'Walk-in'}</td><td className="p-4 font-bold">{settings.currency}{t.total.toFixed(2)}</td><td className="p-4 capitalize">{t.paymentMethod}</td><td className="p-4"><button onClick={() => onPrint(t)} className="p-2 bg-white/20 rounded-lg hover:bg-white/30"><Printer size={16}/></button></td></tr>))}</tbody>
            </table>
        </div>
    </div>
);
...
const App = () => {
    ...
    const [currentUser, setCurrentUser] = useState<User | null>(() => {
        try {
            ...
            // Check Session Storage (Temp Session)
            const session = sessionStorage.getItem('nexus_user');
            return session ? JSON.parse(session) : null;
        } catch { return null; }
    });

    // 3. View State - Persisted active page in Admin dashboard
    const [currentView, setCurrentView] = useState<ViewState>(() => {
        try {
            const storage = localStorage.getItem('nexus_user') ? localStorage : sessionStorage;
            return (storage.getItem('nexus_last_view') as ViewState) || 'DASHBOARD';
    ...
    useEffect(() => {
        if (currentUser) {
            const storage = localStorage.getItem('nexus_user') ? localStorage : sessionStorage;
            storage.setItem('nexus_last_view', currentView);
        }
    ...
    const handleLogin = (user: User, rememberMe: boolean) => {
        ...
        // Save session based on preference
        if (rememberMe) {
        ...
        // Ensure path is /admin
        if (path !== '/admin') navigate('/admin');

        // Check for shift logic
    ...
    const handleLogout = () => {
        ...
        setCurrentUser(null);
        // Force stay on /admin to show login view
        navigate('/admin');
    };

    const handleStartShift = async (amount: number) => {
        if (!currentUser) return;
        const newShift: Partial<Shift> = { userId: currentUser.id, userName: currentUser.name, startTime: new Date().toISOString(), startingCash: amount, status: 'OPEN', cashSales: 0, cardSales: 0, digitalSales: 0, cashMovements: [] };
    ...
    const handleCloseShift = async (countedCash: number) => {
        ...
        const closedShift: Shift = { ...shift, endTime: new Date().toISOString(), status: 'CLOSED', totalSales, expectedCash, countedCash, difference: countedCash - expectedCash };
    ...
    const handleAddProduct = async (p: Partial<Product>) => { const newProd = await api.addProduct(p); setProducts(prev => [...prev, newProd]); setProductModalOpen(false); return newProd; };
    const handleUpdateProduct = async (p: Product) => { const updated = await api.updateProduct(p); setProducts(prev => prev.map(prod => prod.id === updated.id ? updated : prod)); };
    const handleDeleteProduct = async (id: string) => { await api.deleteProduct(id); setProducts(prev => prev.filter(p => p.id !== id)); };
    const handleTransaction = async (t: Transaction) => {
        const saved = await api.saveTransaction(t);
        setTransactions(prev => [saved, ...prev]);
        for (const item of t.items) { const product = products.find(p => p.id === item.id); if (product) { const updated = { ...product, stock: product.stock - item.quantity }; await api.updateProduct(updated); setProducts(prev => prev.map(p => p.id === updated.id ? updated : p)); } }
        if (shift) { const update: Partial<Shift> = { id: shift.id, cashSales: shift.cashSales + (t.paymentMethod === 'cash' ? t.total : 0), cardSales: shift.cardSales + (t.paymentMethod === 'card' ? t.total : 0), digitalSales: shift.digitalSales + (t.paymentMethod === 'digital' ? t.total : 0) }; const updatedShift = await api.saveShift(update); setShift(updatedShift); }
        if (t.customerId) { await api.updateCustomerStats(t.customerId, t.total); setCustomers(await api.getCustomers()); }
    };
    const handleAddWalletMovement = async (type: 'in' | 'out', amount: number, reason: string) => {
        if (!shift || !currentUser) return;
        const movement: CashMovement = { id: Date.now().toString(), type, amount, reason, timestamp: new Date().toISOString(), userId: currentUser.id, userName: currentUser.name };
        const updatedShift = await api.saveShift({ id: shift.id, cashMovements: [...(shift.cashMovements || []), movement] });
    ...
    // 1. Landing Page (Root)
    // If path is root, show landing page regardless of auth status
    if (path === '/' || path === '') {
